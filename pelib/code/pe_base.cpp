...
pe_base::pe_base(std::istream& file, const pe_properties& props, bool read_debug_raw_data)
{
	...
	try {
		...
		readDosHeader(file);
		readPE(file, read_debug_raw_data);
	}
...
void pe_base::realign_section(uint32_t index)
{
	...
	std::size_t strip = 0;
	for(std::size_t i = (*it).getRawData().length(); i >= 1; --i)
	{
		if(s.getRawData()[i - 1] == 0)
			strip++;
		else
			break;
	}
	...
	if(it == sections_.end() - 1) //If we're realigning the last section
	{
		...
		s.set_size_of_raw_data(static_cast<uint32_t>(s.getRawData().length() - strip));
		s.getRawData().resize(s.getRawData().length() - strip, 0);
	}
	else
	{
		...
		s.getRawData().resize(raw_size_aligned, 0);
	}
}
...
bool pe_base::expand_section(section& s, uint32_t needed_rva, uint32_t needed_size, section_expand_type expand)
{
	...
	if(expand == expand_section_raw && section_data_length_from_rva(s, needed_rva, section_data_raw) < needed_size)
	{
		...
		s.getRawData().resize(needed_rva - s.get_virtual_address() + needed_size);
...
void pe_base::prepare_section(section& s)
{
	...
	s.set_size_of_raw_data(static_cast<uint32_t>(pe_utils::align_up(s.getRawData().length(), get_file_alignment())));
...
section& pe_base::add_section(section s)
{
	...
	if(!sections_.empty())
	{
		...
		last.set_size_of_raw_data(static_cast<uint32_t>(pe_utils::align_up(last.getRawData().length(), get_file_alignment())));
	}
	else
	{
...
const char* pe_base::section_data_from_rva(const section& s, uint32_t rva, section_data_type datatype) const
{
	//Check if RVA is inside section "s"
	if(rva >= s.get_virtual_address() && rva < s.get_virtual_address() + s.get_aligned_virtual_size(get_section_alignment()))
		return (datatype == section_data_raw ? s.getRawData().data() : s.get_virtual_data(get_section_alignment()).c_str()) + rva - s.get_virtual_address();
...
uint32_t pe_base::section_data_length_from_rva(uint32_t rva, section_data_type datatype, bool include_headers) const
{
	...
	return static_cast<unsigned long>(datatype == section_data_raw ? s.getRawData().length() /* instead of SizeOfRawData */ : s.get_aligned_virtual_size(get_section_alignment()));
}
...
uint32_t pe_base::section_data_length_from_rva(uint32_t rva, uint32_t rva_inside, section_data_type datatype, bool include_headers) const
{
	...
	long length = static_cast<long>(datatype == section_data_raw ? s.getRawData().length() /* instead of SizeOfRawData */ : s.get_aligned_virtual_size(get_section_alignment())) + s.get_virtual_address() - rva_inside;
...
uint32_t pe_base::section_data_length_from_rva(const section& s, uint32_t rva_inside, section_data_type datatype) const
{
	...
	{
		//Calculate remaining length of section data from "rva" address
		int32_t length = static_cast<int32_t>(datatype == section_data_raw ? s.getRawData().length() /* instead of SizeOfRawData */ : s.get_aligned_virtual_size(get_section_alignment()))
...
void pe_base::readDosHeader(std::istream& file, image_dos_header& header)
{
	//Check istream flags
...
//Reads DOS headers from istream
void pe_base::readDosHeader(std::istream& file)
{
	readDosHeader(file, dos_header_);
}

//Reads PE image from istream
void pe_base::readPE(std::istream& file, bool read_debug_raw_data)
{
	//Get istream size
	...
	for(int i = 0; i < get_number_of_sections(); ++i) {
		...
		if (!pe_utils::is_sum_safe(s.get_virtual_address(), s.get_virtual_size()) || s.get_virtual_size() > pe_utils::two_gb || !pe_utils::is_sum_safe(s.get_pointer_to_raw_data(), s.get_size_of_raw_data()) || s.get_size_of_raw_data() > pe_utils::two_gb) {
		...
		if (s.get_size_of_raw_data() != 0) {
			...
			s.getRawData().resize(s.get_size_of_raw_data());
			file.read(&s.getRawData()[0], s.get_size_of_raw_data());
			if (file.bad() || file.fail()) {
	...
	_hasOverlay = !sections_.empty() && filesize > static_cast<std::streamoff>(sections_.back().get_pointer_to_raw_data() + last_raw_size);

	if (directory_exists(image_directory_entry_security)) {
		file.seekg(static_cast<std::streamoff>(get_directory_rva(image_directory_entry_security)));
		if (file.bad() || file.fail()) {
			throw pe_exception("Cannot reach security data", pe_exception::error_reading_security);
		}

		//Read section raw data
		_security.getRawData().resize(get_directory_size(image_directory_entry_security));
		file.read(&_security.getRawData()[0], get_directory_size(image_directory_entry_security));
		if (file.bad() || file.fail()) {
			throw pe_exception("Error reading security data", pe_exception::error_reading_security);
		}
	}
	...
	while (read_debug_raw_data && has_debug()) {
		try {
			...
			while(directory.PointerToRawData && current_pos < get_directory_rva(image_directory_entry_debug) + get_directory_size(image_directory_entry_debug)) {
				//If we have something to read
				if ((directory.Type == image_debug_type_codeview || directory.Type == image_debug_type_misc || directory.Type == image_debug_type_coff) && directory.SizeOfData) {
					std::string data;
...
bool pe_base::hasSecurity() const
{
	return _security.getRawData().size() > 0;
}
...
void pe_base::recalculate_section_sizes(section& s, bool auto_strip)
{
	...
	if(auto_strip && !(sections_.empty() || &s == &*(sections_.end() - 1)))
	{
		...
		std::string& raw_data = s.getRawData();
		if(!raw_data.empty())
		{
