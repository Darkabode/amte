...
{
	if(s.getRawData().empty()) //Don't count entropy for empty sections
		throw pe_exception("Section is empty", pe_exception::section_is_empty);

	return calculate_entropy(s.getRawData().data(), s.getRawData().length());
}
...
double entropy_calculator::calculate_entropy(const pe_base& pe)
{
	...
	{
		const std::string& data = (*it).getRawData();
