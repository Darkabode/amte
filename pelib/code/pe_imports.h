...
public:
	...
	// Returns hint
	uint16_t getHint() const { return _hint; }
	// Returns ordinal of function
	uint16_t getOrdinal() const { return _ordinal; }
    uint32_t getNameRVA() const { return _nameRVA; }

	// Returns IAT entry VA (usable if image has both IAT and original IAT and is bound)
	uint64_t getIATVA() const { return _iatVA; }
...
public:
	...
	uint32_t getIATRVA() const { return _iatRVA; }
	// Returns RVA to Original Import Address Table (Original IAT)
	uint32_t getINTRVA() const { return _intRVA; }
	// Returns RVA to library name 
	uint32_t getNameRVA() const { return _nameRVA; }
	// Returns timestamp
	uint32_t getTimestamp() const { return _timestamp; }

	//Returns imported functions list
	const imported_list& getImportedFunctions() const { return _imports; }
...
public:
	...
	void setIATRVA(uint32_t rva);
	// Sets RVA to Original Import Address Table (Original IAT)
	void setINTRVA(uint32_t rva);
	// Sets timestamp
	void setTimestamp(uint32_t timestamp);
	// Sets name RVA
	void setNameRVA(uint32_t rva);
...
private:
	...
	uint32_t _iatRVA; //RVA to IAT
	uint32_t _intRVA; //RVA to original IAT
	uint32_t _nameRVA; // RVA to library name
	uint32_t _timestamp; //DLL TimeStamp

	imported_list _imports;
};
