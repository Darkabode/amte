...
void rebuild_pe(pe_base& pe, std::ostream& out, bool strip_dos_header, bool change_size_of_headers, bool save_bound_import)
{
	...
	for(section_list::const_iterator it = sections.begin(); it != sections.end(); ++it) {
		if(it == sections.end() - 1) { //If last section encountered
			...
			header.SizeOfRawData = static_cast<uint32_t>((*it).getRawData().length()); //Set non-aligned actual data length for it
	...
	if (save_bound_import && pe.has_bound_import()) {
		out.write(pe.section_data_from_rva(original_bound_import_rva, section_data_raw, true), pe.get_directory_size(image_directory_entry_bound_import));
	}
	...
	for (section_list::const_iterator it = sections.begin(); it != sections.end(); ++it) {
		...
		out.write(s.getRawData().data(), s.getRawData().length());
	}

	if (pe.hasSecurity()) {
		// Write security data
		out.write(pe.getImageSecurity().getRawData().data(), pe.getImageSecurity().getRawData().length());
	}
}
}
