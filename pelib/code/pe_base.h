...
public: //PE HEADER
	...
	static void readDosHeader(std::istream& file, pe_win::image_dos_header& header);
...
public: //IMAGE SECTIONS
	...
	// Return image security data
	security& getImageSecurity() { return _security; }
	const security& getImageSecurity() const { return _security; }
	...
	T section_data_from_rva(const section& s, uint32_t rva, section_data_type datatype = section_data_raw) const
	{
		...
		{
			const std::string& data = datatype == section_data_raw ? s.getRawData() : s.get_virtual_data(get_section_alignment());
...
private:
	...
	//Reads and checks DOS header
	void readDosHeader(std::istream& file);

	//Reads and checks PE headers and section headers, data
	void readPE(std::istream& file, bool read_debug_raw_data);
