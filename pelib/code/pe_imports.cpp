...
bool imported_function::hasName() const
{
	return !_name.empty();
}

//Sets name of function
void imported_function::setName(const std::string& name, uint32_t nameRVA)
{
	_name = name;
    _nameRVA = nameRVA;
}
...
//Sets hint
void imported_function::setHint(uint16_t hint)
{
	_hint = hint;
}

//Sets ordinal
void imported_function::setOrdinal(uint16_t ordinal)
{
	_ordinal = ordinal;
}

//Sets IAT entry VA (usable if image has both IAT and original IAT and is bound)
void imported_function::setIATVA(uint64_t va)
{
	_iatVA = va;
}
...
void import_library::setName(const std::string& name)
{
	_name = name;
}

//Sets RVA to Import Address Table (IAT)
void import_library::setIATRVA(uint32_t rva)
{
	_iatRVA = rva;
}

//Sets RVA to Original Import Address Table (Original IAT)
void import_library::setINTRVA(uint32_t rva)
{
	_intRVA = rva;
}

//Sets timestamp
void import_library::setTimestamp(uint32_t timestamp)
{
	_timestamp = timestamp;
}

void import_library::setNameRVA(uint32_t rva)
{
	_nameRVA = rva;
}
...
const imported_functions_list get_imported_functions_base(const pe_base& pe)
{
	...
	while (import_descriptor.Name) {
		...
		//Set library name
		lib.setName(dll_name);
		//Set library timestamp
		lib.setTimestamp(import_descriptor.TimeDateStamp);
		//Set library RVA to IAT and original IAT
		lib.setIATRVA(import_descriptor.FirstThunk);
		lib.setINTRVA(import_descriptor.OriginalFirstThunk);
		lib.setNameRVA(import_descriptor.Name);
		...
		if (import_lookup_table != 0 && import_address_table != 0) {
			while (true) {
				...
				if ((lookup & PEClassType::ImportSnapFlag) != 0) {
					...
					//Save hint and name
					func.setName(func_name, (uint32_t)lookup);
					func.setHint(hint);
				}
...
//You can get all image imports with getImportedFunctions() function
...
const image_directory rebuild_imports_base(pe_base& pe, const imported_functions_list& imports, section& import_section, const import_rebuilder_settings& import_settings)
{
	...
	{
		needed_size_for_strings += static_cast<uint32_t>((*it).getName().length() + 1 /* nullbyte */);

		const import_library::imported_list& funcs = (*it).getImportedFunctions();
		...
		{
			if((*f).hasName())
				needed_size_for_strings += static_cast<uint32_t>((*f).getName().length() + 1 /* nullbyte */ + sizeof(uint16_t) /* hint */);
		}
	}
	...
	std::string& raw_data = import_section.getRawData();
	...
	for(imported_functions_list::const_iterator it = imports.begin(); it != imports.end(); ++it)
	{
		...
		descr.TimeDateStamp = (*it).getTimestamp(); //Restore timestamp
		...
		bool save_iats_for_this_descriptor = import_settings.save_iat_and_original_iat_rvas() && (*it).getIATRVA() != 0;
		...
		bool rewrite_saved_iat = save_iats_for_this_descriptor && import_settings.rewrite_iat_and_original_iat_contents() && (*it).getIATRVA() != 0;
		...
		if(save_iats_for_this_descriptor)
		{
			...
			if(!(*it).getINTRVA() && import_settings.fill_missing_original_iats())
				...
				descr.OriginalFirstThunk = import_settings.build_original_iat() ? (*it).getINTRVA() : 0;
			
			descr.FirstThunk = (*it).getIATRVA();
		...
		//Save library name
		memcpy(&raw_data[current_string_pointer], (*it).getName().c_str(), (*it).getName().length() + 1 /* nullbyte */);
		current_string_pointer += static_cast<uint32_t>((*it).getName().length() + 1 /* nullbyte */);
		
		//List all imported functions
		const import_library::imported_list& funcs = (*it).getImportedFunctions();
		for(import_library::imported_list::const_iterator f = funcs.begin(); f != funcs.end(); ++f)
		{
			if((*f).hasName()) //If function is imported by name
			{
				...
				if(!save_iats_for_this_descriptor)
				{
					if(write_original_iat)
					{
						...
						typename PEClassType::BaseSize iat_value = static_cast<typename PEClassType::BaseSize>((*f).getIATVA());
				...
				uint16_t hint = (*f).getHint();
				memcpy(&raw_data[current_string_pointer], &hint, sizeof(hint));
				memcpy(&raw_data[current_string_pointer + sizeof(uint16_t)], (*f).getName().c_str(), (*f).getName().length() + 1 /* nullbyte */);
				current_string_pointer += static_cast<uint32_t>((*f).getName().length() + 1 /* nullbyte */ + sizeof(uint16_t) /* hint */);
			}
			else //Function is imported by ordinal
			{
				uint16_t ordinal = (*f).getOrdinal();
